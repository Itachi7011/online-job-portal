...
function ByLocation() {
    ...
    return (<>
        ...
        <div className="m-5 searching ">
            ...
            <h2 className="searchtext1">Search Jobs by Location (Address) : </h2>
            ...
            {Data.post.filter((field) => {
                if (User.searchinput === "") {
                    return 0;
                } else if (field.fullAddress.toLocaleLowerCase().includes(User.searchinput.toLocaleLowerCase()) ||
                    field.state.toLocaleLowerCase().includes(User.searchinput.toLocaleLowerCase())
                    ||
                    field.country.toLocaleLowerCase().includes(User.searchinput.toLocaleLowerCase())) {
                    return field;
                }
            })
                .map(({ CompanyName, salaryExpected, _id, field, email, phoneNo, fullAddress, state, startingYear, minQualifications, minExperienceNeeded, noOfEmpNeeded }) => {
